...
var AnnotationDataAccess = (function () {
    function AnnotationDataAccess() {
    }
    AnnotationDataAccess.prototype.retrieveTestProcedureDocumentAnnotation = function (testProcedureDocumentId) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            var annotation = new MobileCRM.FetchXml.Entity('annotation');
            ...
            annotationFilter.where('objectid', 'eq', testProcedureDocumentId);
            annotation.filter.filters.push(annotationFilter);
            var featureFetch = new MobileCRM.FetchXml.Fetch(annotation);
            featureFetch.execute('JSON', function (testProcedureDocAnnotations) {
                if (testProcedureDocAnnotations && testProcedureDocAnnotations.length > 0) {
                    var promises = new Array();
                    for (var i = 0; i < testProcedureDocAnnotations.length; i++) {
                        promises.push(_this.promiseGetAnnotationDocument(testProcedureDocAnnotations[i]));
                    }
                    _this.getAllAnnotations(promises).then(function (annotations) {
                        resolve(annotations);
                    ...
                    reject('No test procedure document annotation found for testproceduredocument: ' + testProcedureDocumentId);
                }
            }, function (err) {
                reject(err);
            }, null);
    ...
    AnnotationDataAccess.prototype.retrieveAnnotationsByObjectId = function (objectId) {
        return new Promise(function (resolve, reject) {
            var annotation = new MobileCRM.FetchXml.Entity('annotation');
            annotation.addAttributes();
            annotation.filter = new MobileCRM.FetchXml.Filter();
            ...
            annotation.filter.filters.push(annotationFilter);
            var featureFetch = new MobileCRM.FetchXml.Fetch(annotation);
            featureFetch.execute('JSON', function (annotations) {
                resolve(annotations);
            }, function (err) {
                reject(err);
            }, null);
    ...
    AnnotationDataAccess.prototype.retrieveAnnotationsWithBodyByObjectId = function (objectId) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            var annotation = new MobileCRM.FetchXml.Entity('annotation');
            annotation.addAttributes();
            ...
            featureFetch.execute('JSON', function (annotations) {
                ...
                annotations.forEach(function (a) {
                    promises.push(_this.promiseGetAnnotationDocument(a));
                });
                _this.getAllAnnotations(promises).then(function (annotations) {
                    resolve(annotations);
                }).catch(function (error) {
    ...
    AnnotationDataAccess.prototype.retrieveTestProcedureDocumentAnnotations = function (testProcedure, language) {
        var _this = this;
        var testProcedureId = testProcedure.dw_testprocedureid;
        var languageId = language.dw_languageid;
        return new Promise(function (resolve, reject) {
            var annotation = new MobileCRM.FetchXml.Entity('annotation');
            annotation.addAttributes();
            var testProcedureDocument = annotation.addLink('dw_testproceduredocument', 'dw_testproceduredocumentid', 'objectid', 'inner');
            testProcedureDocument.addAttribute('dw_testproceduredocumentid');
            testProcedureDocument.addAttribute('dw_testprocedureid');
            var procedure = testProcedureDocument.addLink('dw_testprocedure', 'dw_testprocedureid', 'dw_testprocedureid', 'inner');
            testProcedureDocument.filter = new MobileCRM.FetchXml.Filter();
            var testProcedureDocumentFilter = new MobileCRM.FetchXml.Filter();
            testProcedureDocumentFilter.where('dw_language', 'eq', languageId);
            testProcedureDocument.filter.filters.push(testProcedureDocumentFilter);
            procedure.filter = new MobileCRM.FetchXml.Filter();
            var testProcedureFilter = new MobileCRM.FetchXml.Filter();
            testProcedureFilter.where('dw_testprocedureid', 'eq', testProcedureId);
            ...
            featureFetch.execute('JSON', function (testProcedureDocAnnotations) {
                if (testProcedureDocAnnotations && testProcedureDocAnnotations.length > 0) {
                    ...
                    reject('No test procedure document annotation found for testprocedure: ' + testProcedureId);
                }
            }, function (err) {
                reject(err);
            }, null);
    ...
    AnnotationDataAccess.prototype.retrieveTestStepDocumentAnnotations = function (testProcedureDocumentId) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            var annotation = new MobileCRM.FetchXml.Entity('annotation');
            annotation.addAttributes();
            var teststepDocument = annotation.addLink('dw_teststepdocument', 'dw_teststepdocumentid', 'objectid', 'inner');
            teststepDocument.addAttribute('dw_link');
            var testProcedureDocTestStepDoc = teststepDocument.addLink('dw_testproceduredoc_teststepdoc', 'dw_teststepdocumentid', 'dw_teststepdocumentid', 'inner');
            var testProcedureDoc = testProcedureDocTestStepDoc.addLink('dw_testproceduredocument', 'dw_testproceduredocumentid', 'dw_testproceduredocumentid', 'inner');
            testProcedureDoc.addAttribute('dw_testproceduredocumentid');
            testProcedureDoc.filter = new MobileCRM.FetchXml.Filter();
            var testProcedureDocumentFilter = new MobileCRM.FetchXml.Filter();
            testProcedureDocumentFilter.where('dw_testproceduredocumentid', 'eq', testProcedureDocumentId);
            testProcedureDoc.filter.filters.push(testProcedureDocumentFilter);
            var featureFetch = new MobileCRM.FetchXml.Fetch(annotation);
            featureFetch.execute('JSON', function (testStepDocumentAnnotations) {
                if (testStepDocumentAnnotations && testStepDocumentAnnotations.length > 0) {
                    var promises = new Array();
                    for (var i = 0; i < testStepDocumentAnnotations.length; i++) {
                        promises.push(_this.promiseGetAnnotationDocument(testStepDocumentAnnotations[i]));
                    }
                    _this.getAllAnnotations(promises).then(function (annotations) {
                        resolve(annotations);
                    }).catch(function (error) {
    ...
    AnnotationDataAccess.prototype.retrieveTestStepResultDocumentAnnotationsForServiceOrderNumber = function (serviceOrderNumber) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            ...
            var annotation = new MobileCRM.FetchXml.Entity('annotation');
            annotation.addAttributes();
            var testStepResultDocumentLink = annotation.addLink('dw_teststepresultdocument', 'dw_teststepresultdocumentid', 'objectid', 'inner');
            var resultHeaderLink = testStepResultDocumentLink.addLink('dw_teststepresultheader', 'dw_teststepresultheaderid', 'dw_resultheaderid', 'inner');
            var serviceOrderLink = resultHeaderLink.addLink('dw_serviceorder', 'dw_serviceorderid', 'dw_serviceorderid', 'inner');
            serviceOrderLink.addAttribute('dw_serviceordernumber');
            serviceOrderLink.filter = new MobileCRM.FetchXml.Filter();
            serviceOrderLink.filter.where('dw_serviceordernumber', 'eq', serviceOrderNumber);
            var featureFetch = new MobileCRM.FetchXml.Fetch(annotation);
            featureFetch.execute('JSON', function (testStepResultDocumentAnnotations) {
                if (testStepResultDocumentAnnotations && testStepResultDocumentAnnotations.length > 0) {
                    var promises = new Array();
                    for (var i = 0; i < testStepResultDocumentAnnotations.length; i++) {
                        promises.push(_this.promiseGetAnnotationDocument(testStepResultDocumentAnnotations[i]));
                    }
                    _this.getAllAnnotations(promises).then(function (annotations) {
                        resolve(annotations);
                    }).catch(function (error) {
    ...
    AnnotationDataAccess.prototype.retrieveEquipmentHistoryAnnotations = function (installedBaseIds) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            ...
            var annotation = new MobileCRM.FetchXml.Entity('annotation');
            annotation.addAttributes();
            var equipmentHistory = annotation.addLink('dw_equipmenthistory', 'dw_equipmenthistoryid', 'objectid', 'inner');
            var equipment = equipmentHistory.addLink('speed1_installedbase', 'speed1_installedbaseid', 'dw_equipmentid', 'inner');
            ...
            equipmentFilter.isIn('speed1_installedbaseid', installedBaseIds);
            equipment.filter.filters.push(equipmentFilter);
            var featureFetch = new MobileCRM.FetchXml.Fetch(annotation);
            featureFetch.execute('JSON', function (equipmentHistoryAnnotations) {
                if (equipmentHistoryAnnotations && equipmentHistoryAnnotations.length > 0) {
                    var promises = new Array();
                    for (var i = 0; i < equipmentHistoryAnnotations.length; i++) {
                        promises.push(_this.promiseGetAnnotationDocument(equipmentHistoryAnnotations[i]));
                    }
                    _this.getAllAnnotations(promises).then(function (annotations) {
                        resolve(annotations);
                    }).catch(function (error) {
    ...
    AnnotationDataAccess.prototype.saveAnnotation = function (annotation, entityName) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            var attachment = null;
            if (annotation.AnnotationId != null &&
                annotation.AnnotationId !== '00000000-0000-0000-0000-000000000000') {
                fileoperations_1.writeAnnotationToFile(annotation).then(function (filePath) {
                    _this.promiseSaveAnnotationDocument(annotation, filePath, entityName).then(function () {
                        fileoperations_1.deleteTwaFile(annotation.FileName);
                        return resolve(true);
                    }).catch(function (error) {
                        return reject(error);
                    });
                ...
                attachment = MobileCRM.DynamicEntity.createNew('annotation');
                var properties = attachment.properties;
                var referObject = new MobileCRM.Reference(entityName, annotation.ObjectId);
    ...
    AnnotationDataAccess.prototype.deleteAnnotation = function (annotation) {
        return new Promise(function (resolve, reject) {
            MobileCRM.DynamicEntity.deleteById('annotation', annotation.AnnotationId, function () {
                return resolve(true);
            }, function (error) {
                return reject(error);
            }, null);
    ...
    AnnotationDataAccess.prototype.getAllAnnotations = function (annotationPromises) {
        return new Promise(function (resolve, reject) {
    ...
    AnnotationDataAccess.prototype.promiseGetAnnotationDocument = function (annotation) {
        return new Promise(function (resolve, reject) {
            MobileCRM.DynamicEntity.loadDocumentBody('annotation', annotation.annotationid, function (base64String) {
    ...
    AnnotationDataAccess.prototype.promiseSaveAnnotationDocument = function (annotation, filePath, relatedEntityName) {
        return new Promise(function (resolve, reject) {
            var referObject = new MobileCRM.Reference(relatedEntityName, annotation.ObjectId);
            var relationship = new MobileCRM.Relationship("objectid", referObject);
            MobileCRM.DynamicEntity.saveDocumentBody(annotation.AnnotationId, null, relationship, filePath, null, function () {
                resolve(true);
            }, function (error) {
                reject(error);
            });
    ...
    AnnotationDataAccess.prototype.retrieveEquipmentHistoryTransformedXMLByInstalledBaseIfExists = function (installedBaseId, fileNamePart) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            ...
            equipment.filter = new MobileCRM.FetchXml.Filter();
            var equipmentFilter = new MobileCRM.FetchXml.Filter();
            equipmentFilter.where('speed1_installedbaseid', 'eq', installedBaseId);
            equipment.filter.filters.push(equipmentFilter);
            var annotationFetch = new MobileCRM.FetchXml.Fetch(annotation);
            annotationFetch.execute('JSON', function (equipmentHistoryAnnotation) {
                if (equipmentHistoryAnnotation != null && equipmentHistoryAnnotation.length > 0) {
                    for (var i = 0; i < equipmentHistoryAnnotation.length; i++) {
                        if (equipmentHistoryAnnotation[i].filename.indexOf(fileNamePart) != -1) {
                            _this.promiseExtractAnnotationZipContent(equipmentHistoryAnnotation[i]).then(function () {
                                resolve(true);
                            }).catch(function (error) {
                                reject(error);
                            });
    ...
    AnnotationDataAccess.prototype.promiseExtractAnnotationZipContent = function (annotation) {
        return new Promise(function (resolve, reject) {
            MobileCRM.DynamicEntity.unzipDocumentBody('annotation', annotation.annotationid, "EquipmentHistoryXML", function () {
                resolve();
            }, function (error) {
                reject(error);
            });
...
/***/ "./src/configuration/entitytrackingstate.ts":
/*!**************************************************!*\
  !*** ./src/configuration/entitytrackingstate.ts ***!
  \**************************************************/
/*! no static exports found */
...
Object.defineProperty(exports, "__esModule", { value: true });
var EntityTrackingState;
(function (EntityTrackingState) {
    EntityTrackingState[EntityTrackingState["New"] = 1] = "New";
    EntityTrackingState[EntityTrackingState["Existing"] = 2] = "Existing";
    EntityTrackingState[EntityTrackingState["Added"] = 3] = "Added";
    EntityTrackingState[EntityTrackingState["Deleted"] = 4] = "Deleted";
    EntityTrackingState[EntityTrackingState["Updated"] = 5] = "Updated";
})(EntityTrackingState = exports.EntityTrackingState || (exports.EntityTrackingState = {}));
...
/***/ "./src/models/dataobjects/twateststepattachment.ts":
/*!*********************************************************!*\
  !*** ./src/models/dataobjects/twateststepattachment.ts ***!
  \*********************************************************/
/*! no static exports found */
...
Object.defineProperty(exports, "__esModule", { value: true });
var TwaTestStepAttachment = (function () {
    function TwaTestStepAttachment() {
    }
    return TwaTestStepAttachment;
}());
...
/***/ "./src/models/dataobjects/twateststepresult.ts":
/*!*****************************************************!*\
  !*** ./src/models/dataobjects/twateststepresult.ts ***!
  \*****************************************************/
/*! no static exports found */
...
Object.defineProperty(exports, "__esModule", { value: true });
var TwaTestStepResult = (function () {
    function TwaTestStepResult() {
    }
    return TwaTestStepResult;
}());
...
/***/ "./src/teststepresult/teststepresult.ts":
/*!**********************************************!*\
  !*** ./src/teststepresult/teststepresult.ts ***!
  \**********************************************/
/*! no static exports found */
...
Object.defineProperty(exports, "__esModule", { value: true });
var entitytrackingstate_1 = __webpack_require__(/*! ../configuration/entitytrackingstate */ "./src/configuration/entitytrackingstate.ts");
var TestStepResult = (function () {
    function TestStepResult() {
    }
    TestStepResult.prototype.retrieveTestStepResults = function (resultHeaderIds) {
        return new Promise(function (resolve, reject) {
            var entity = new MobileCRM.FetchXml.Entity('dw_teststepresult');
    ...
    TestStepResult.prototype.createTestStepResult = function (twaTestStepResult) {
        return new Promise(function (resolve, reject) {
            var testStepResult = null;
            testStepResult = MobileCRM.DynamicEntity.createNew('dw_teststepresult');
            var properties = testStepResult.properties;
            var referResultHeader = new MobileCRM.Reference('dw_teststepresultheader', twaTestStepResult.ResultHeaderId);
            var referTestStep = new MobileCRM.Reference('dw_teststep', twaTestStepResult.TestStepId);
            properties.dw_comment = twaTestStepResult.Comment;
            properties.dw_divisionmt = true;
            properties.dw_divisionst = false;
            properties.dw_editablelistvalue = twaTestStepResult.ListItemText;
            if (twaTestStepResult.ResultCompleted != null) {
                properties.dw_resultcompleted = twaTestStepResult.ResultCompleted ? 1 : 0;
            }
            else {
                properties.dw_resultcompleted = null;
            }
            if (twaTestStepResult.IsLocked != null) {
                properties.dw_islocked = twaTestStepResult.IsLocked ? 1 : 0;
            }
            properties.dw_resultdate = twaTestStepResult.ResultDate;
            properties.dw_resultmeasurement = twaTestStepResult.ResultMeasurement;
            ...
            testStepResult.save(function (error) {
                ...
                var testStepResultReturn = {
                    ...
                    dw_editablelistvalue: testStepResult.properties.dw_editablelistvalue,
                    dw_islocked: testStepResult.properties.dw_islocked,
                    dw_listitemtext: testStepResult.properties.dw_listitemtext,
                    modifiedby: testStepResult.properties.modifiedby,
                    dw_resultcompleted: testStepResult.properties.dw_resultcompleted,
                    dw_resultdate: testStepResult.properties.dw_resultdate,
                    dw_resultfile: testStepResult.properties.dw_resultfile,
                    dw_resultmeasurement: testStepResult.properties.dw_resultmeasurement,
                    dw_resulttext: testStepResult.properties.dw_resulttext,
                    dw_teststepresultheaderid: testStepResult.properties.dw_teststepresultheaderid,
                    dw_teststepid: testStepResult.properties.dw_teststepid,
                    dw_teststepresultid: testStepResult.properties.dw_teststepresultid,
                    CurrentEntityTrackingState: entitytrackingstate_1.EntityTrackingState.Added,
                };
                return resolve(testStepResultReturn);
            });
        });
    };
    TestStepResult.prototype.updateTestStepResult = function (twaTestStepResult) {
        return new Promise(function (resolve, reject) {
            var testStepResult = null;
            testStepResult = new MobileCRM.DynamicEntity('dw_teststepresult', twaTestStepResult.TestStepResultId);
            var properties = testStepResult.properties;
            properties.dw_comment = twaTestStepResult.Comment;
            properties.dw_editablelistvalue = twaTestStepResult.ListItemText;
            if (twaTestStepResult.ResultCompleted != null) {
            ...
            testStepResult.save(function (error) {
                ...
                var testStepResultReturn = {
                    ...
                    dw_teststepid: testStepResult.properties.dw_teststepid,
                    dw_teststepresultid: testStepResult.properties.dw_teststepresultid,
                    CurrentEntityTrackingState: entitytrackingstate_1.EntityTrackingState.Updated,
                };
                return resolve(testStepResultReturn);
            });
        });
    };
    TestStepResult.prototype.deleteTestStepResult = function (twaTestStepResult) {
        return new Promise(function (resolve, reject) {
            MobileCRM.DynamicEntity.deleteById('dw_teststepresult', twaTestStepResult.TestStepResultId, function () {
                return resolve(true);
            }, function (error) {
                return reject(error);
            }, null);
...
/***/ "./src/teststepresultdocument/teststepresultdocument.ts":
/*!**************************************************************!*\
  !*** ./src/teststepresultdocument/teststepresultdocument.ts ***!
  \**************************************************************/
/*! no static exports found */
...
Object.defineProperty(exports, "__esModule", { value: true });
var annotation_1 = __webpack_require__(/*! ../annotation/annotation */ "./src/annotation/annotation.ts");
var TestStepResultDocument = (function () {
    function TestStepResultDocument() {
    }
    TestStepResultDocument.prototype.retrieveTestStepResultDocuments = function (resultHeaderIds) {
        return new Promise(function (resolve, reject) {
            var entity = new MobileCRM.FetchXml.Entity('dw_teststepresultdocument');
            entity.addAttributes();
            entity.filter = new MobileCRM.FetchXml.Filter();
    ...
    TestStepResultDocument.prototype.createTestStepResultDocument = function (twaTestStepAttachment) {
        return new Promise(function (resolve, reject) {
            var testStepAttachment = null;
            testStepAttachment = MobileCRM.DynamicEntity.createNew('dw_teststepresultdocument');
            var properties = testStepAttachment.properties;
            var referTestStepResult = new MobileCRM.Reference('dw_teststepresult', twaTestStepAttachment.TestStepResultId);
            var referResultHeader = new MobileCRM.Reference('dw_teststepresultheader', twaTestStepAttachment.ResultHeaderId);
            properties.dw_filename = twaTestStepAttachment.TestStepResultDocumentFileName;
            properties.dw_resultheaderid = referResultHeader;
            properties.dw_teststepresultid = referTestStepResult;
            var annotationDataAccess = new annotation_1.AnnotationDataAccess();
            testStepAttachment.save(function (error) {
                ...
                var attachment = {
                    ...
                    ObjectId: testStepAttachment.properties.dw_teststepresultdocumentid,
                };
                annotationDataAccess.saveAnnotation(attachment, 'dw_teststepresultdocument').then(function () {
                    var testStepResultDocument = {
                        createdby: testStepAttachment.properties.createdby,
                        dw_filename: testStepAttachment.properties.dw_filename,
                        modifiedby: testStepAttachment.properties.modifiedby,
                        dw_resultheaderid: testStepAttachment.properties.dw_resultheaderid,
                        dw_teststepresultid: testStepAttachment.properties.dw_teststepresultid,
                        dw_teststepresultdocumentid: testStepAttachment.properties.dw_teststepresultdocumentid
                    };
    ...
    TestStepResultDocument.prototype.updateTestStepResultDocument = function (twaTestStepAttachment) {
        return new Promise(function (resolve, reject) {
            var testStepAttachment = null;
            testStepAttachment = new MobileCRM.DynamicEntity('dw_teststepresultdocument', twaTestStepAttachment.TestStepAttachmentId);
            var properties = testStepAttachment.properties;
            properties.dw_filename = twaTestStepAttachment.TestStepResultDocumentFileName;
            testStepAttachment.save(function (error) {
                if (error) {
    ...
    TestStepResultDocument.prototype.deleteTestStepResultDocument = function (twaTestStepAttachment) {
        return new Promise(function (resolve, reject) {
            MobileCRM.DynamicEntity.deleteById('dw_teststepresultdocument', twaTestStepAttachment.TestStepAttachmentId, function () {
                return resolve(true);
            }, function (error) {
                return reject(error);
            }, null);
...
/***/ "./src/twa/twa-savetestresults.ts":
/*!****************************************!*\
  !*** ./src/twa/twa-savetestresults.ts ***!
  \****************************************/
/*! no static exports found */
...
Object.defineProperty(exports, "__esModule", { value: true });
__webpack_require__(/*! js-polyfills */ "./node_modules/js-polyfills/polyfill.js");
var annotation_1 = __webpack_require__(/*! ../annotation/annotation */ "./src/annotation/annotation.ts");
var entitytrackingstate_1 = __webpack_require__(/*! ../configuration/entitytrackingstate */ "./src/configuration/entitytrackingstate.ts");
var filenames_1 = __webpack_require__(/*! ../configuration/filenames */ "./src/configuration/filenames.ts");
var fileoperations_1 = __webpack_require__(/*! ../fileoperations/fileoperations */ "./src/fileoperations/fileoperations.ts");
var fileoperations_2 = __webpack_require__(/*! ../fileoperations/fileoperations */ "./src/fileoperations/fileoperations.ts");
var twateststepattachment_1 = __webpack_require__(/*! ../models/dataobjects/twateststepattachment */ "./src/models/dataobjects/twateststepattachment.ts");
var twateststepresult_1 = __webpack_require__(/*! ../models/dataobjects/twateststepresult */ "./src/models/dataobjects/twateststepresult.ts");
var teststepresult_1 = __webpack_require__(/*! ../teststepresult/teststepresult */ "./src/teststepresult/teststepresult.ts");
var teststepresultdocument_1 = __webpack_require__(/*! ../teststepresultdocument/teststepresultdocument */ "./src/teststepresultdocument/teststepresultdocument.ts");
var TwaSaveTestResults = (function () {
    function TwaSaveTestResults() {
    }
    TwaSaveTestResults.prototype.saveTestResults = function () {
        this.getTestStepResults()
            .then(this.saveTestStepResults.bind(this))
            .then(this.getTestStepAttachments.bind(this))
            .then(this.saveTestStepAttachments.bind(this))
            .then(function () {
    ...
    TwaSaveTestResults.prototype.deleteTwaFiles = function (fileNames) {
        return new Promise(function (resolve, reject) {
            var promises = new Array();
    ...
    TwaSaveTestResults.prototype.getTestStepResults = function () {
        return new Promise(function (resolve, reject) {
            var twaTestStepResults = [];
            fileoperations_2.readFromJsonFile(filenames_1.FileNames.TwaTestStepResults)
                .then(function (testStepResults) {
                for (var i = 0; i < testStepResults.length; i++) {
                    var twaTestStepResult = new twateststepresult_1.TwaTestStepResult();
                    twaTestStepResult.Comment = testStepResults[i].Comment;
                    twaTestStepResult.CurrentEntityTrackingState = testStepResults[i].CurrentEntityTrackingState;
                    twaTestStepResult.IsAvailableInCrm = testStepResults[i].IsAvailableInCrm;
                    twaTestStepResult.IsLocked = testStepResults[i].IsLocked;
                    twaTestStepResult.ListItemText = testStepResults[i].ListItemText;
                    twaTestStepResult.ResultCompleted = testStepResults[i].ResultCompleted;
                    twaTestStepResult.ResultDate = testStepResults[i].ResultDate;
                    twaTestStepResult.ResultHeaderId = testStepResults[i].ResultHeaderId;
                    twaTestStepResult.ResultMeasurement = testStepResults[i].ResultMeasurement;
                    twaTestStepResult.ResultText = testStepResults[i].ResultText;
                    twaTestStepResult.TestStepId = testStepResults[i].TestStepId;
                    twaTestStepResult.TestStepResultId = testStepResults[i].TestStepResultId;
                    twaTestStepResult.ServiceOrderId = testStepResults[i].ServiceOrderId;
                    twaTestStepResults.push(twaTestStepResult);
                }
                resolve(twaTestStepResults);
            }).catch(function (error) {
                reject(error);
            });
        });
    };
    TwaSaveTestResults.prototype.saveTestStepResults = function (twaTestStepResults) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            if (twaTestStepResults == null || twaTestStepResults.length === 0) {
                var fileNames = [];
                fileNames.push(filenames_1.FileNames.TwaTestStepResults);
                _this.deleteTwaFiles(fileNames);
                resolve(null);
            }
            var promiseRequests = [];
            var testStepResult = new teststepresult_1.TestStepResult();
            twaTestStepResults.forEach(function (twaTestStepResult) {
                if (twaTestStepResult.CurrentEntityTrackingState === entitytrackingstate_1.EntityTrackingState.Added) {
                    promiseRequests.push(testStepResult.createTestStepResult(twaTestStepResult));
                }
                if (twaTestStepResult.CurrentEntityTrackingState === entitytrackingstate_1.EntityTrackingState.Updated) {
                    promiseRequests.push(testStepResult.updateTestStepResult(twaTestStepResult));
                }
                if (twaTestStepResult.CurrentEntityTrackingState === entitytrackingstate_1.EntityTrackingState.Deleted) {
                    promiseRequests.push(testStepResult.deleteTestStepResult(twaTestStepResult));
                }
            });
            Promise.all(promiseRequests)
                .then(function (results) {
                if (results != null && results.length > 0) {
                    var testStepResults_1 = Array();
                    results.forEach(function (result) {
                        if (result !== true) {
                            testStepResults_1.push(result);
                        }
                    });
                    var resultsToSave_1 = [];
                    testStepResults_1.forEach(function (result) {
                        if (result.CurrentEntityTrackingState === entitytrackingstate_1.EntityTrackingState.Added) {
                            resultsToSave_1.push(result);
                        }
                    });
                    if (resultsToSave_1 != null && resultsToSave_1.length > 0) {
                        fileoperations_1.saveToJsonFile(resultsToSave_1, filenames_1.FileNames.TwaTestStepResults);
                    }
                    else {
                        var fileNames = [];
                        fileNames.push(filenames_1.FileNames.TwaTestStepResults);
    ...
    TwaSaveTestResults.prototype.saveTestStepAttachments = function (twaTestStepAttachments) {
        var _this = this;
        return new Promise(function (resolve, reject) {
            var promiseRequests = [];
            var promiseSaveRequests = [];
            var testStepResultDocument = new teststepresultdocument_1.TestStepResultDocument();
            twaTestStepAttachments.forEach(function (twaTestStepAttachment) {
                if (twaTestStepAttachment.CurrentEntityTrackingState === entitytrackingstate_1.EntityTrackingState.Deleted) {
                    promiseRequests.push(testStepResultDocument.deleteTestStepResultDocument(twaTestStepAttachment));
                    promiseRequests.push(_this.deleteAnnotationByTestProcedureDocumentId(twaTestStepAttachment));
                }
                if (twaTestStepAttachment.CurrentEntityTrackingState === entitytrackingstate_1.EntityTrackingState.New ||
                    twaTestStepAttachment.CurrentEntityTrackingState === entitytrackingstate_1.EntityTrackingState.Added) {
                    promiseSaveRequests.push(testStepResultDocument.createTestStepResultDocument(twaTestStepAttachment));
                }
                if (twaTestStepAttachment.CurrentEntityTrackingState === entitytrackingstate_1.EntityTrackingState.Updated) {
                    promiseSaveRequests.push(testStepResultDocument.updateTestStepResultDocument(twaTestStepAttachment));
                    promiseRequests.push(_this.updateAnnotationByTestProcedureDocumentId(twaTestStepAttachment));
                }
            });
            Promise.all(promiseRequests)
                .then(function () {
                Promise.all(promiseSaveRequests)
                    .then(function (results) {
                    if (results != null && results.length > 0) {
                        var resultsToSave_2 = [];
                        results.forEach(function (result) {
                            if (result !== true) {
                                resultsToSave_2.push(result);
                            }
                        });
                        if (resultsToSave_2 != null && resultsToSave_2.length > 0) {
                            fileoperations_1.saveToJsonFile(resultsToSave_2, filenames_1.FileNames.TwaTestStepAttachments);
                        }
                        else {
                            var fileNames = [];
                            fileNames.push(filenames_1.FileNames.TwaTestStepAttachments);
                            _this.deleteTwaFiles(fileNames);
                        }
                    }
    ...
    TwaSaveTestResults.prototype.updateAnnotationByTestProcedureDocumentId = function (twaTestStepAttachment) {
        return new Promise(function (resolve, reject) {
            var annotationPromises = new Array();
            var annotationDataAccess = new annotation_1.AnnotationDataAccess();
            annotationDataAccess.retrieveAnnotationsByObjectId(twaTestStepAttachment.TestStepAttachmentId).then(function (annotations) {
                for (var x = 0; x < annotations.length; x++) {
                    var attachment = {
                        AnnotationId: annotations[x].annotationid,
                        DocumentBody: twaTestStepAttachment.Body,
                        FileName: twaTestStepAttachment.FileName,
                        ObjectId: twaTestStepAttachment.TestStepAttachmentId,
                    };
                    annotationPromises.push(annotationDataAccess.saveAnnotation(attachment, 'dw_teststepresultdocument'));
                }
    ...
    TwaSaveTestResults.prototype.deleteAnnotationByTestProcedureDocumentId = function (twaTestStepAttachment) {
        return new Promise(function (resolve, reject) {
            var annotationPromises = new Array();
            var annotationDataAccess = new annotation_1.AnnotationDataAccess();
            annotationDataAccess.retrieveAnnotationsByObjectId(twaTestStepAttachment.TestStepAttachmentId).then(function (annotations) {
                for (var x = 0; x < annotations.length; x++) {
                    var attachment = {
                        ...
                        DocumentBody: twaTestStepAttachment.Body,
                        FileName: twaTestStepAttachment.FileName,
                        TestStepResultDocumentId: twaTestStepAttachment.TestStepAttachmentId,
                    };
                    annotationPromises.push(annotationDataAccess.deleteAnnotation(attachment));
                }
                Promise.all(annotationPromises)
                    .then(function (result) {
                    return resolve(result);
    ...
    TwaSaveTestResults.prototype.getTestStepAttachments = function (teststepResults) {
        return new Promise(function (resolve, reject) {
            var twaTestStepAttachments = [];
            fileoperations_2.readFromJsonFile(filenames_1.FileNames.TwaTestStepAttachments)
                .then(function (testStepAttachments) {
                for (var i = 0; i < testStepAttachments.length; i++) {
                    var twaTestStepAttachment = new twateststepattachment_1.TwaTestStepAttachment();
                    var testStepId = testStepAttachments[i].TestStepId;
                    var trackingState = testStepAttachments[i].CurrentEntityTrackingState;
                    twaTestStepAttachment.FileName = testStepAttachments[i].FileName;
                    twaTestStepAttachment.TestStepResultDocumentFileName = testStepAttachments[i].TestStepResultDocumentFileName;
                    twaTestStepAttachment.CurrentEntityTrackingState = testStepAttachments[i].CurrentEntityTrackingState;
                    twaTestStepAttachment.ResultHeaderId = testStepAttachments[i].ResultHeaderId;
                    twaTestStepAttachment.Body = testStepAttachments[i].Body;
                    if (trackingState == entitytrackingstate_1.EntityTrackingState.Updated || trackingState == entitytrackingstate_1.EntityTrackingState.Deleted) {
                        twaTestStepAttachment.TestStepAttachmentId = testStepAttachments[i].TestStepAttachmentId;
                        twaTestStepAttachments.push(twaTestStepAttachment);
                        continue;
                    }
                    if (trackingState == entitytrackingstate_1.EntityTrackingState.Added && testStepAttachments[i].TestStepResultId != null
                        && testStepAttachments[i].TestStepResultId !== '00000000-0000-0000-0000-000000000000') {
                        twaTestStepAttachment.TestStepResultId = testStepAttachments[i].TestStepResultId;
                        twaTestStepAttachments.push(twaTestStepAttachment);
                        continue;
                    }
                    for (var a = 0; a < teststepResults.length; a++) {
                        var element = teststepResults[a];
                        if (element.dw_teststepid.id === testStepId && element.dw_teststepresultheaderid.id === testStepAttachments[i].ResultHeaderId) {
                            twaTestStepAttachment.TestStepResultId = element.dw_teststepresultid;
                            twaTestStepAttachments.push(twaTestStepAttachment);
                        }
                    }
                }
                resolve(twaTestStepAttachments);
            }).catch(function (error) {
                reject(error);
            });
        });
    };
    return TwaSaveTestResults;
}());
exports.TwaSaveTestResults = TwaSaveTestResults;
...
/***/ 3:
/*!**********************************************!*\
  !*** multi ./src/twa/twa-savetestresults.ts ***!
  \**********************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

module.exports = __webpack_require__(/*! C:\Users\torsten.kayser\source\repos\D365\Source\Project\RescoClient\src\twa\twa-savetestresults.ts */"./src/twa/twa-savetestresults.ts");


/***/ })

/******/ });
//# sourceMappingURL=twa_savetestresults.resco.js.map